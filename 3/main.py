...
def build_experiment_grid() -> List[Dict[str, Any]]:
    ...
    reptree_params_l = [3, 5, -1];
    reptree_params_m = [2, 5, 10]
    ...
    randomtree_params_k = [0, 2, 5];
    randomtree_params_m = [1.0, 10, 20]
...
def apply_numeric_to_nominal_filter(input_arff: Path, output_arff: Path, java_cmd_base: list) -> bool:
    ...
    cmd_filter = java_cmd_base + ["weka.filters.unsupervised.attribute.NumericToNominal", "-R", "last", "-i",
...
def generate_visualization_for_config(config: Dict[str, Any], main_file: Path, java_cmd_base: List[str],
    ...
    if output_dot_source:
        ...
        print(
            f"--- Failed to get .dot source for {config['name']}, skipping visualization (this is expected for REPTree). ---")


def generate_rules_for_config(config: Dict[str, Any], main_file: Path, java_cmd_base: List[str],
                              output_filename_base: str):
...
def main():
    ...
    for prefix in DATASET_PREFIXES:
        ...
        if not csv_file.is_file():
            print(f"FATAL: Source CSV file '{csv_file}' not found. Run build_dataset.py first.");
            continue
        ...
        if not apply_numeric_to_nominal_filter(temp_arff_file, final_arff_file, java_cmd_base):
            print(f"FATAL: Could not create final filtered ARFF file. Skipping.");
            continue
        ...
        for config in full_experiment_grid:
            ...
            cmd_cv = java_cmd_base + [classifier_class, "-t", final_arff_file, "-c", "last", "-x",
            ...
            cmd_split = java_cmd_base + [classifier_class, "-t", final_arff_file, "-c", "last", "-split-percentage",
            ...
            cmd_model_summary = java_cmd_base + [classifier_class, "-t", final_arff_file, "-c", "last",
                                                 "-no-cv"] + classifier_params
            ...
            if family_name in best_models_by_family:
                ...
                if current_cv > best_so_far["cv_score"] or \
                        ('tree_size' in best_so_far and current_cv == best_so_far["cv_score"] and current_size <
                         best_so_far["tree_size"]):
                    print(f"*** New best {family_name} model found: {config['name']} (CV: {current_cv}%) ***")
                    best_models_by_family[family_name] = {"config": config, "cv_score": current_cv,
        ...
        for algo_name, default_config_params in DEFAULT_ALGORITHM_CONFIG.items():
            ...
            if "trees" in default_config['class']:
                generate_visualization_for_config(default_config, final_arff_file, java_cmd_base,
                                                  f"{prefix}_{algo_name}_default")
                ...
                if best_model and best_model.get("config"):
                    ...
                    if best_config["params"] != default_config["params"]:
                        generate_visualization_for_config(best_config, final_arff_file, java_cmd_base,
                                                          f"{prefix}_{algo_name}_optimised")
                    else:
                        print(
                            f"--- Best {algo_name} model was the default. Skipping duplicate optimised visualization. ---")

            elif "rules" in default_config['class']:
                generate_rules_for_config(default_config, final_arff_file, java_cmd_base,
                                          f"{prefix}_{algo_name}_default_rules")

                best_model = best_models_by_family.get(algo_name)
                if best_model and best_model.get("config"):
                    best_config = best_model["config"]
                    if best_config["params"] != default_config["params"]:
                        generate_rules_for_config(best_config, final_arff_file, java_cmd_base,
                                                  f"{prefix}_{algo_name}_optimised_rules")
                    else:
                        print(
                            f"--- Best {algo_name} model was the default. Skipping duplicate optimised rules file. ---")
